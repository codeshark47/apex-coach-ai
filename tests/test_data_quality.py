...
numbers can be trusted, based purely on how many of the 7 metrics came
...
def _all_metrics_present():
    """A metrics dict where all 7 metrics genuinely computed — every
    ...
    return {
        ...
        "rear_knee_angle": {"degrees": 175.0, "status": "success"},
        "rear_hip_flexion": {"degrees": 10.0, "status": "success"},
    }


class TestAssessQuality:
    def test_all_seven_present_is_high_confidence_zero_missing(self):
    ...
    def test_three_missing_metrics_stays_high_confidence(self):
        """LOW_CONFIDENCE_THRESHOLD is 4 (out of 7) — exactly 3 missing
        must NOT trip low confidence (boundary test, one below the
        threshold)."""
        metrics = _all_metrics_present()
    ...
    def test_four_missing_metrics_is_low_confidence(self):
        """Boundary test at the threshold itself (4 out of 7)."""
        metrics = _all_metrics_present()
        metrics["trunk_lean"]["degrees"] = None
        metrics["hip_shoulder_separation"]["degrees"] = None
        metrics["release_height"]["ratio"] = None
        metrics["rear_knee_angle"]["degrees"] = None
        result = dq.assess_quality(metrics)
        assert result["missing_count"] == 4
        assert result["confidence"] == "low"

    def test_all_seven_missing_is_low_confidence(self):
        metrics = {
            ...
            "rear_knee_angle": {"degrees": None},
            "rear_hip_flexion": {"degrees": None},
        }
        result = dq.assess_quality(metrics)
