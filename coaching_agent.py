...
- RECALIBRATION-PENDING RULE: if ANY metric above is marked "[RECALIBRATION PENDING]", its underlying measurement was just corrected to fix a real false-reading bug, but the OPTIMAL/ACCEPTABLE/CRITICAL bands it's compared against were tuned for the OLD measurement and have not been re-validated for the new one yet. You may still report the number and its ZONE as useful, directional information, but explicitly note in the narrative that this specific reading is provisional pending re-validation, and do NOT prescribe a drill based on this metric alone even if its ZONE reads CRITICAL.
  EXCEPTION (2026-09-23, explicit coaching-staff decision): Head Stability specifically is exempted from the "do NOT prescribe a drill" part of this rule — its CRITICAL threshold is still provisional, but the coaching staff has judged the underlying signal directionally trustworthy enough to act on. If Head Stability is marked [RECALIBRATION PENDING] and its ZONE is CRITICAL, you MAY prescribe a drill for it. You must still explicitly note in the narrative that this specific reading is provisional pending full re-validation — this exception changes drill eligibility only, never the disclosure obligation. Every other RECALIBRATION-PENDING metric (e.g. Release Height) still follows the rule above exactly with no drill.
...
FIRST, build the real eligible-metric list: a metric qualifies ONLY if its ZONE is CRITICAL AND it is not marked DESCRIPTIVE or TRACKING-UNCERTAIN anywhere above, AND (it is not marked RECALIBRATION-PENDING OR it is Head Stability — see the RECALIBRATION-PENDING RULE's exception above, the ONLY metric this applies to). (A real bug in an actual report: this exact scenario occurred with Hip-Shoulder Separation DESCRIPTIVE — 3 drills got prescribed for it anyway. That must never happen again.)
