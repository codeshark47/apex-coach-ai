...
def extract_video_landmarks(video_path: str, output_csv_path: str) -> dict:
    ...
    # Using explicit context handling for the MediaPipe resource
    ...
    # Pre-build our structured data matrix structure
    columns = ["frame"]
    ...
    while True:
        ...
        # Consistent Horizontal Cropping Window (Dynamic relative bounds)
        left = int(w * 0.2)
        right = int(w * 0.8)
        cropped = frame[:, left:right]
        cropped_w = right - left

        # Prepare image for perception model processing
        frame_rgb = cv2.cvtColor(cropped, cv2.COLOR_BGR2RGB)
        results = pose.process(frame_rgb)

        # Base frame metadata assignment
        row = [frame_number]

        if results.pose_landmarks:
            for landmark in results.pose_landmarks.landmark:
                # Re-map crop-space X coordinates back into standardized native image scale
                global_x = (landmark.x * cropped_w + left) / w
                row.extend([global_x, landmark.y, landmark.z])
        else:
            # Enforce structural padding: fill row with NaNs if detection drops
            # Each landmark possesses 3 dimensions (x, y, z) across 33 trackpoints
    ...
    # Save data structure safely using pandas to ensure correct encoding
